...
Code review (standards + spec axes) led to: the unit spec on the failure body dropped, since the body is proved at Seam 2 and the spec allows exactly three seams, with the no-field case added there; the Seam 1 spec kept to the parse boundary, no longer asserting `requiredFieldsFor` directly; the form importing the shared `CONDITIONAL_REQUIREMENT` marker instead of re-spelling it, and the unused `role` dropped from the issue's params; the Role's article read from a table beside the Role's fields rather than a second Role-keyed branch; a server message about a field the form does not have carried in the notice, where before it landed on the form root and was never shown; `aria-required` on a control the chosen Role requires; the API spec's Ada-without-fields setup as one helper. Left as-is on purpose: a `server` error outliving a Role change until its own field is edited, which needs the server and the browser to disagree, which the shared schema prevents; whitespace-only phoneNumber reported as malformed rather than missing, the same on both sides; the fourth browser spec proving the Role following and the error's placement beside its line (story 59) as well as the admin case, since the browser is the only seam that can.
