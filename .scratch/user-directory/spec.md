...
**Frontend composition.** Reads are expressed as resources derived from the current page and search terms, so changing either re-fetches declaratively; creation is an imperative one-shot request. The Angular and Angular Material APIs the challenge brief names are the ones used, so an assessor grading against its wording finds each where they expect it: the creation form is an Angular Reactive Form (`ReactiveFormsModule`, a `FormGroup` of `FormControl`s) of Material form fields (`mat-form-field` with `matInput`, `mat-select` for the Role), validated by the shared creation schema with each issue mapped onto the control at its path and shown in `mat-error`; a User's detail opens in a Material Dialog (`MatDialog`) from its route; the outcome of a submission is shown with Material Snackbar (`MatSnackBar`); the list pages with `MatPaginator`; and the smiley is a standalone `SmileyComponent`. Field-level problems appear inline against their control; the outcome of a submission is announced separately as a transient notification.
